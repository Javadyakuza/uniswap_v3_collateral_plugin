...
/// @notice every pair in a uniswap pool will have a seperate ERC-20 token but all of them are wrapperaable by this wrapper contract. 
 interface IUNI_V3_POS_Wrapper {
     ...
     * @return _whole_wUNI_V3_POSPrice whole value of the whole amount wUNI_V3_POS. 
     */
    function wUNI_V3_POSPrice() external view returns(uint192 _wUNI_V3_POSPrice, uint192 _whole_wUNI_V3_POSPrice);
    /**
     * @notice this function deposites all of the earned fee's on a tokenId to the "_recipient".
     * @param _sender the actuall transaction caller from collateral contract
     ...
     */
    function claimRewards(address _sender, uint256 _positionId, address _recipient) external returns(address token0, uint256 amount0, address token1, uint256 amount1);
    /**
     ...
     */
    function pairTokenInformation(address token0, address token1) external view returns(
      uint256 amount0,
      uint256 amount1,
      int24 tickLowest,
      int24 tickUppermost,
    ...
    /**
     * @notice via this function we can set the intialing price range (tickLower and tickUpper)
     * @param _token0 ETH-address of the token0
     * @param _token1 ETH-address of the token1
     * @param _tickLower //
     * @param _tickUpper // 
     */
    function setPairTokenTicks(address _token0, address _token1, uint24 _tickLower, uint24 _tickUpper) external; 
